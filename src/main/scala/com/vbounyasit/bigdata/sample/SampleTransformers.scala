...
class SampleTransformers extends Transformers {
  ...
  case class FilterOnTable1() extends Transformer {
    override val transform: DataFrame => DataFrame = _.where(col("col3") === "text2")
  }

  case class MultiplyByFactor(columnName: String, factor: Int) extends Transformer {
    override val transform: DataFrame => DataFrame = _.withColumn(columnName, col(columnName) * factor)
  }

  case class MultiplyByColumn(columnName: String, multiplyByColumn: String) extends Transformer {
    override val transform: DataFrame => DataFrame = _.withColumn(columnName, col(columnName) * col(multiplyByColumn))
  }

  case class SumByColumn(keyColumn: String, columnToSum: String) extends Transformer {
    override val transform: DataFrame => DataFrame = _.groupBy(keyColumn).agg(sum(columnToSum).as(s"sum_of_$columnToSum"))
  }

  case class KeepGreaterThan(columnName: String, thresholdValue: Int) extends Transformer {
    override val transform: DataFrame => DataFrame = _.where(col(columnName) >= thresholdValue)
  }

  case class RenameColumn(columnName: String, newColumnName: String) extends Transformer {
    override val transform: DataFrame => DataFrame = _.withColumnRenamed(columnName, newColumnName)
  }
}